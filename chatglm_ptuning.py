...
# ChatGLM_Ptuning class: initialize by PtuningType to use one model with multiple fine-tuned weights.
class ChatGLM_Ptuning(LLM, ABC):
    ...
    # Load different fine-tuned weights according to the requested task type.
    def __init__(self, ptuning_type: PtuningType):
        ...
        if ptuning_type is PtuningType.Classify or ptuning_type is PtuningType.NL2SQL or ptuning_type is PtuningType.Keywords:
            # For classification, load CLASSIFY weights with sequence length 512.
            ...
            # For keyword extraction, load KEYWORDS weights with sequence length 256.
            ...
            # For NL2SQL, load weights with sequence length 2048.
                ...
                self.model = AutoModel.from_pretrained(self.model_name, config=self.config, trust_remote_code=True)
                self.isNL2SQL = True
            # Apply the weights from the corresponding checkpoint path.
...
用户输入：'''
    ...
    # Unload currently attached weighted model components.
