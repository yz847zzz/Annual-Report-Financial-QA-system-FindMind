...
def get_match_pdf_names(question, pdf_info):
    ...
    # The year has already been fully matched above, so remove it here.
    ...
    if len(match_keys) > 1:
        ...
        # Multiple candidates have identical overlap scores.
...
def parse_keyword_from_answer(anoy_question, answer):
    ...
    for key_word in key_word_list:
        ...
        # key_word = re.sub('annual report|report|whether', '', key_word)
...
def parse_question_keywords(model, question, real_company, years):
    ...
    # If extraction fails, retry after removing noisy content.
    if len(key_words) == 0:
